...
public class TdsClientLobOkTest {
  ...
  public Mono<Void> runSql(Connection connection) {
    ...
    String q4 = "SET TEXTSIZE -1; SELECT REPLICATE(CAST('A' AS VARCHAR(MAX)), 2500000) AS LargeString;";
    String q5 = "SET TEXTSIZE -1; SELECT CAST(REPLICATE(CAST('A' AS VARCHAR(MAX)), 2500000) AS VARBINARY(MAX)) AS LargeBinary;";
    String q6 = "SET TEXTSIZE -1; SELECT REPLICATE(CAST(N'あ' AS NVARCHAR(MAX)), 2500000) AS LargeNString;";
