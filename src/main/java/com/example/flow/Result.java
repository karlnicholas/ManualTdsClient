 ...
 *
 * <p>A {@link io.r2dbc.spi.Result} object maintains a consumption state that may be backed by a cursor pointing
 * to its current row of data or out parameters.  A {@link io.r2dbc.spi.Result} allows read-only and forward-only consumption of statement results.
...
public interface Result {
   ...
   * <p>Consuming the update count may emit an {@link Subscriber#onError(Throwable) error signal} if the results contain one or more {@link io.r2dbc.spi.Result.Message error message segments}.
   *
   ...
   * <p>Consuming data rows may emit an {@link Subscriber#onError(Throwable) error signal} if the results contain one or more {@link io.r2dbc.spi.Result.Message error message segments}.
   *
  ...
  /**
   * Returns a mapping of the rows/out parameters that are the results of a query against a database.  May be empty if the query did not return any results.  A {@link io.r2dbc.spi.Readable} can be only
   * considered valid within a {@link Function mapping function} callback.
   *
   * @param mappingFunction the {@link Function} that maps a {@link io.r2dbc.spi.Readable} to a value
   * @param <T>             the type of the mapped value
   * @return a mapping of the rows that are the results of a query against a database
   * @throws IllegalArgumentException if {@code mappingFunction} is {@code null}
   * @throws IllegalStateException    if the result was consumed
...
//    Assert.requireNonNull(mappingFunction, "mappingFunction must not be null");
  ...
  /**
   * Returns a filtered variant of the {@link io.r2dbc.spi.Result} to selectively consume result segments matching {@link Predicate filter predicate}.
   * <p>The returned {@link io.r2dbc.spi.Result} is a potentially reduced view of the underlying {@link io.r2dbc.spi.Result} to filter out unwanted result segments.  For example, filtering all {@link io.r2dbc.spi.Result.Message} segments
   * from the result
   ...
   * @return a filtered {@link io.r2dbc.spi.Result}
   * @throws IllegalArgumentException if {@code filter} is {@code null}
   * @throws IllegalStateException    if the result was consumed
   * @since 0.9
   */
  io.r2dbc.spi.Result filter(Predicate<io.r2dbc.spi.Result.Segment> filter);

  /**
   * Returns a mapping of the {@link io.r2dbc.spi.Result.Segment result segments} that are the results of a query against a database.  May be empty if the query did not return any segments.  A {@link io.r2dbc.spi.Result.Segment} can be
   * only considered valid within a {@link Function mapping function} callback.
   * <p>Consuming result {@link io.r2dbc.spi.Result.Segment segments} does not emit {@link Subscriber#onError(Throwable) error signals} from {@link io.r2dbc.spi.Result.Message message segments representing an error}.  Translation of
   ...
   *
   * @param mappingFunction the {@link Function} that maps a {@link io.r2dbc.spi.Result.Segment} a to a {@link Publisher}
   * @param <T>             the type of the mapped value
   ...
   * @since 0.9
   */
  <T> Publisher<T> flatMap(Function<io.r2dbc.spi.Result.Segment, ? extends Publisher<? extends T>> mappingFunction);
   ...
   * @since 0.9
   */
  interface RowSegment extends io.r2dbc.spi.Result.Segment {
   ...
   * @since 0.9
   */
  interface OutSegment extends io.r2dbc.spi.Result.Segment {
   ...
   * @since 0.9
   */
  interface UpdateCount extends io.r2dbc.spi.Result.Segment {
   ...
   * @since 0.9
   */
  interface Message extends io.r2dbc.spi.Result.Segment {
