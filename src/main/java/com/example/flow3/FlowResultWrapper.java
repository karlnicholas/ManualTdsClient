...
public class FlowResultWrapper<T> implements Flow.Publisher<T> {
  ...
  public <R> FlowResultWrapper<R> flatMap(Function<T, ? extends Flow.Publisher<? extends R>> nextMapper) {
    // This requires the wrapper to handle the transition from the T emitted by the current
    // mapper back into a new Publisher flow.
    return new FlowResultWrapper<>(source, i -> {
      // This logic gets complex because FlowResult.flatMap expects
      // an Integer input, but your wrapper is currently at type T.
      throw new UnsupportedOperationException("FlatMap requires state-aware flattening");
    });
  }
