...
public class TdsClientPlpVarchar {
  public static void main(String[] args) {
    new TdsClientPlpVarchar().run();
  }
  ...
  private Mono<Void> test_PlpStream(Connection connection, String executionLabel, int targetSize) {
    return Mono.defer(() -> {
      ...
      String sql = "SELECT REPLICATE(CAST('A' AS VARCHAR(MAX)), " + targetSize + ") AS plp_col";
      ...
      return Flux.from(connection.createStatement(sql).execute())
          ...
          .flatMap(res -> res.map((row, metadata) -> row.get(0, Clob.class)))
          ...
          .flatMap(clob -> {
            ...
            return Flux.from(clob.stream())
                ...
                .reduce(0, (totalLength, chunk) -> totalLength + chunk.length())
