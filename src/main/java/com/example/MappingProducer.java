...
public class MappingProducer<T> implements Publisher<T> {
  private final Publisher<T> source;

  private MappingProducer(Publisher<T> source) {
    this.source = source;
  }
  ...
  public <R> MappingProducer<R> map(Function<T, R> mapper) {
    Publisher<R> mappedSource = downstream -> source.subscribe(new Subscriber<T>() {
      @Override
  ...
  // -----------------------------------------------------------
  public <R> MappingProducer<R> flatMap(Function<T, Publisher<R>> mapper) {
  ...
  // -----------------------------------------------------------
  private static class FlatMapSubscriber<T, R> implements Subscriber<T>, Subscription {
    ...
    private final AtomicReference<Subscription> innerSubscription = new AtomicReference<>();
    ...
    FlatMapSubscriber(Subscriber<? super R> downstream, Function<T, Publisher<R>> mapper) {
    ...
    public void request(long n) {
      ...
      Subscription inner = innerSubscription.get();
      if (inner != null) {
        inner.request(n);
    ...
    public void onNext(T item) {
      ...
      try {
        Publisher<R> innerPub = mapper.apply(item);

        // Subscribe to the inner publisher
        innerPub.subscribe(new Subscriber<R>() {
          @Override
          public void onSubscribe(Subscription s) {
